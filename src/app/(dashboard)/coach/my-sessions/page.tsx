...
export default function MySessionsPage() {
  ...
  const canJoinSession = (scheduledFor: string | Date) => {
    ...
    const earlyAccess = new Date(sessionTime.getTime() - 24 * 60 * 60 * 1000); // 24 hours early
  ...
  return (
    <div className="space-y-8 min-h-screen bg-[#F8FAFC] p-6 rounded-[2.5rem]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {!isLearner && (
          <Link href={ROUTES.COACH.CREATE_SESSION} className="cursor-pointer">
      ...
      <Card className="border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-white/80 backdrop-blur-xl">
        ...
        <CardContent className="p-8 pt-0">
          {isLoading ? (
          ...
          ) : (isLearner ? bookings.length > 0 : (slots.length > 0 || bookings.length > 0)) ? (
            <div className="space-y-12">
              {/* Scheduled Sessions Section (For Coaches) */}
              {!isLearner && bookings.length > 0 && (
                <div className="space-y-6">
                  <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em]">Scheduled Mentorships</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {bookings.map((booking: PopulatedBooking) => (
                      <div key={booking._id} className="bg-white border border-slate-100 p-6 rounded-[2rem] hover:border-blue-200 transition-all group shadow-sm flex flex-col">
                        <div className="flex items-center space-x-4 mb-5">
                          <Avatar className="h-12 w-12 rounded-2xl ring-2 ring-slate-100">
                            <AvatarImage src={booking.learnerId.profileImage} />
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-slate-500 text-sm font-medium">Learner: {booking.learnerId.firstName} {booking.learnerId.lastName}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-auto pt-5 border-t border-slate-100 flex justify-between items-center">
                          ...
                          {booking.sessionStatus === 'scheduled' && (
                            <Button 
                              ...
                              disabled={!canJoinSession(booking.scheduledFor)}
                              className={`font-black rounded-xl px-5 py-5 flex items-center space-x-2 transition-all shadow-md active:scale-95 ${
                                canJoinSession(booking.scheduledFor) 
                                ? 'bg-green-600 hover:bg-green-700 text-white' 
                                : 'bg-slate-100 text-slate-400'
                              }`}
                            >
                              {canJoinSession(booking.scheduledFor) ? <Video className="h-4 w-4" /> : <VideoOff className="h-4 w-4" />}
                              <span>{canJoinSession(booking.scheduledFor) ? 'Start Session' : 'Not Ready'}</span>
                            </Button>
                          )}
                        </div>
                      </div>
              ...
              {/* Standard List View (Learner or Coach Availability) */}
              <div className="space-y-6">
                {!isLearner && <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em]">Your Availability Slots</h3>}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {isLearner ? (
                    bookings.map((booking: PopulatedBooking) => (
                      <div key={booking._id} className="bg-slate-50/50 border border-slate-100 p-6 rounded-[2rem] hover:border-blue-200 transition-all group shadow-sm flex flex-col">
                        <div className="flex items-center space-x-4 mb-5">
                          <Avatar className="h-12 w-12 rounded-2xl ring-2 ring-white">
                            <AvatarImage src={booking.coachId.profileImage} />
                    ...
                    slots.map((slot) => (
                      <div key={slot._id} className="bg-white border border-slate-100 p-6 rounded-[2rem] hover:border-blue-200 transition-all group shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                          <h4 className="text-lg font-black text-slate-900">{slot.title}</h4>
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
                          <div className="text-2xl font-black text-slate-900">${slot.price}</div>
                          <div className="text-slate-500 text-[10px] font-black uppercase tracking-widest flex items-center">
            ...
            <div className="text-center py-20 bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-200">
              <div className="bg-white w-20 h-20 rounded-3xl shadow-lg flex items-center justify-center mx-auto mb-6 transform hover:rotate-6 transition-transform">
              ...
              {isLearner ? (
                ...
                <Link href={ROUTES.COACH.CREATE_SESSION}>
      ...
      {isLearner && bookings.length > 0 && (
        <Card className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 border-0 shadow-2xl text-white rounded-[2.5rem]">
          <CardContent className="p-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex-1">
              <h3 className="text-3xl font-black mb-3 flex items-center">
                <Sparkles className="w-8 h-8 mr-4 text-blue-300" />
            ...
            <div className="flex gap-4">
              <Button 
                onClick={() => router.push(ROUTES.LEARNER.EXPLORE)}
                className="bg-white text-blue-600 hover:bg-slate-50 font-black py-7 px-8 rounded-2xl text-lg shadow-xl active:scale-95 transition-transform"
              >
                Find Coaches
              </Button>
            </div>
